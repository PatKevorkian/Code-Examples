...
//history array to store history commands
...
//Fuction to get the command from shell

int formatCommand(char inputBuffer[], char *args[],int *flag)
{
    // # of chars in command line
        int length;
    // loop index for inputBuffer
        int i;
    // loop index for inputBuffer
    	int start;
    // index of beginning of next command
    	int ct = 0;
    ...
    for (i=0;i<length;i++)
    {
        switch (inputBuffer[i])
        {
            ...
            case '\t' :               // to seperate arguments
                ...
                inputBuffer[i] = '\0'; // add null char at end
                start = -1;
                break;
            ...
            default :           
                ...
                if (inputBuffer[i] == '&')
                {
                    *flag  = 1; //flag for if child process is invoked or in background
                    inputBuffer[i] = '\0';
                }
        }
    }
