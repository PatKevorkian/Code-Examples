...
% Mean value
%filterType = averagefilter(image, window, padding);

%Median value
filterType = medfilt2(image);

% Standard deviation
meanSquare = averagefilter(image.^2, window, padding);
deviation = (meanSquare - filterType.^2).^0.5;

% Sauvola
R = max(deviation(:));
threshold = filterType.*(1 + k * (deviation / R-1));
%threshold = filterType + (k * deviation);
