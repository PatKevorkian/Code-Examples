...
clear all;
close all;
mkdir('Results2//'); %Directory for Storing Results

%% Configurations/Parameters
dataFileName = 'spiral.dat';
nbrOfNeuronsInEachHiddenLayer = [15 10]; %linear:[4] - circle:[10] - wave,spirals:[10 10]
nbrOfOutUnits = 2;
unipolarBipolarSelector = 0; %0 for Unipolar, -1 for Bipolar

learningRate = 0.2;
nbrOfEpochs_max = 10000;
...
%% Read Data
importedData = importdata(dataFileName, '\t', 200);
...
for Epoch = 1:nbrOfEpochs_max
    ...
    if (zeroRMSReached || mod(Epoch,draw_each_nbrOfEpochs)==0)
        ...
        saveas(gcf, sprintf('Results2//fig%i.png', Epoch),'jpg');
        pause(0.05);
    end
    ...
    if (zeroRMSReached)
        saveas(gcf, sprintf('Results2//Final Result for %s.png', dataFileName),'jpg');
        break;
    end
